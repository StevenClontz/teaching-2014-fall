...
%  Adding up the terms in an infinite sequence is a \textbf{series}.  That is to say, given a sequence $\left(a_n\right)$, the series would be denoted as $$\sum_{i = 1}^{\infty} a_i.$$
% \end{definition}

% \begin{definition}[Partial Sum\index{Partial Sum}]
%  Let $\left( a_n \right)$ be a sequence.  The \textbf{partial sums} are
%  $$\begin{array}{rclcl}
%     s_1 &=& a_1 \\
%     s_2 &=& a_1 + a_2 \\
%     s_3 &=& a_1 + a_2 + a_3 \\
%     \vdots \\
%     s_m &=& a_1 + a_2 + a_3 &=& \ds \sum_{i = 1}^{m} a_i.
%    \end{array}$$
% \end{definition}

% \begin{definition}[Series Convergence \& Sum\index{Series Convergence}\index{Series Divergence}\index{Series Sum}]
%  Given a series $\sum_{i = 1}^{\infty} a_i$, let $s_n$ be the $n^{\rm th}$ partial sum.  If the sequence $\left( s_n \right)$ converges (that is $\lim_{n \rightarrow \infty} s_n = s$), then the series $\sum_{i = 1}^{\infty} a_n$ is said to \textbf{converge} and we write $$\sum_{i = 1}^{\infty} a_n = s.$$  The number $s$ is called the \textbf{sum} of the series.  Moreover, if $\left( s_n \right)$ diverges, then the series is also said to be \textbf{divergent}.
% \end{definition}

% \newpage

% \begin{problem}
%  Determine whether or not the series whose first $n$ terms sum to $\ds s_n = a_1 + a_2 + \cdots + a_n = \frac{2n}{3n+5}$ converges or diverges. \\ \emph{Note} This does not mean that our series is $\ds \sum_{n = 1}^{\infty} \frac{2n}{3n+5}$.  To find the sequence, one would have to find $a_1, a_2, a_3, \ldots$ and find the pattern, which is a very difficult problem in general.
% \end{problem}
...
% \begin{problem}
%  Find the sum of the series $\ds \sum_{n = 0}^{\infty} x^n$ for $\left|x\right| < 1$.
% \end{problem}

% \vfill

% \begin{problem}
%  Show that $\ds \sum_{n = 1}^{\infty} \frac{1}{n\left(n+1\right)}$ converges and find its sum.  \emph{Hint} Look at the partial sums.
% \end{problem}
...
% \begin{problem}
%  Show that $\ds \sum_{n = 1}^{\infty} \frac{n^2}{5n^2 + 4}$ diverges.
% \end{problem}
...
% \begin{observation}
%  Let the sequence $\left(a_n\right)$ is defined to be integer points on a positive, continuous, non-increasing function $f$ (that is points are labeled as $(n, a_n) = (n, f(n))$ where $n \in \mathbb{N}$).  Then if we were to examine the area under the curve with the same technique as we did with Riemann Sums, we would see that the width of each rectangle would be $1$, and the height would be $f(n)$ for each point $n$.  Moreover, the area of each rectangle is $f(n)$ and the summation of that sequence as a series would be the summation of all of those rectangles.  This {\sc Does Not}, however, give us the true summation of the sequence, since we are not allowing these rectangles to be infinitely close together within each integer point.  The best we can say is that the series converges if and only if the integral converges.
% \end{observation}
...
%  Suppose that $f$ is a continuous, positive, non-increasing function on $[1, \infty)$ such that $a_n = f(n)$.  Then the series $\sum_{n=1}^{\infty} a_n$ is convergent if and only if $\int_1^\infty f(x) \, dx = \lim_{t \rightarrow \infty} \int_1^t f(x) \, dx = L$ is convergent (that is the limit exists and $L \in \mathbb{R}$ and $L \neq \pm \infty$).
% \end{theorem}

% \begin{problem}
%  Does $\ds \sum_{n=1}^{\infty} \frac{1}{n^2+1}$ converge or diverge?
% \end{problem}
...
% \begin{problem}
%  Determine whether $\ds \sum_{n=1}^{\infty} \frac{\ln\left(n\right)}{n}$ converges or diverges.
% \end{problem}

% \vfill

% \noindent Suggested Problems: Section $11.3$ numbers $3,$ $4,$ $5,$ $11,$ $12,$ $13,$ $15,$ $25$
...
% \begin{problem}
%  Determine whether $\ds \sum_{n = 1}^{\infty} \frac{1}{2^n-1}$ converges or diverges.
% \end{problem}

% \vfill

% \begin{problem}
%  Determine whether $\ds \sum_{n=1}^{\infty} \frac{2n^2+3n}{\sqrt{5+n^5}}$ converges or diverges.
% \end{problem}
...
% \begin{theorem}[Absolute $\Rightarrow$ Conditional]
%  If a series $\sum a_n$ is absolutely convergent, then it is also conditionally convergent.
% \end{theorem}

% \begin{problem}
%  Determine whether whether or not $\ds \sum_{n=1}^{\infty} \frac{\left(-1\right)^{n-1}}{n^2}$ is absolutely convergent.  Is it conditionally convergent?
% \end{problem}

% \vfill

% \begin{problem}
%  Determine whether whether or not $\ds \sum_{n=1}^{\infty} \frac{\left(-1\right)^{n-1}}{n}$ is absolutely convergent.  Is it conditionally convergent?
% \end{problem}

% \vfill

% \begin{problem}
%  Determine whether whether or not $\ds \sum_{n=1}^{\infty} \frac{\cos\left(n\right)}{n^2}$ is absolutely convergent.  Is it conditionally convergent?
% \end{problem}
...
%  \begin{itemize}
%   \item if $\ds \lim_{n \rightarrow \infty} \left| \frac{a_{n+1}}{a_{n}} \right| = L < 1,$ then the series $\sum_{n = 1}^{\infty} a_n$ is absolutely (and therefore conditionally) convergent.
%   \item if $\ds \lim_{n \rightarrow \infty} \left| \frac{a_{n+1}}{a_{n}} \right| = L > 1$ or $\ds \lim_{n \rightarrow \infty} \left| \frac{a_{n+1}}{a_{n}} \right| = \infty$ then the series $\sum_{n = 1}^{\infty} a_n$ is divergent.
%   \item if $\ds \lim_{n \rightarrow \infty} \left| \frac{a_{n+1}}{a_{n}} \right| = 1,$ then no conclusion can be drawn from this test.
%  \end{itemize}
% \end{theorem}
...
%  Let $\sum_{n=1}^{\infty} a_n$ be a series.  Then
%  \begin{itemize}
%   \item If $\ds \lim_{n \rightarrow \infty} \sqrt[n]{\left|a_n\right|} = L < 1,$ then $\sum_{n=1}^{\infty} a_n$ is absolutely (and therefore conditionally) convergent.
%   \item if $\ds \lim_{n \rightarrow \infty} \sqrt[n]{\left|a_n\right|} = L > 1$ or $\ds \lim_{n \rightarrow \infty} \sqrt[n]{\left|a_n\right|} = \infty$ then the series $\sum_{n = 1}^{\infty} a_n$ is divergent.
...
% \begin{problem}
%  For what values of $x$ is the series $\ds \sum_{n=1}^{\infty} \frac{\left(x-3\right)^n}{n}$ converge?
% \end{problem}
